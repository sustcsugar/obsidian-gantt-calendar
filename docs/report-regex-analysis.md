...
| **链接处理正则 (硬编码)** |
| 33 | `obsidianLinkRegex` | `\[\[([^\]|]+)(?:\|([^\]]+))?\]\]` | g | BaseViewRenderer.ts | 312 | `"[[Note]]"` | 匹配Obsidian双向链接 | ❌ |
| 34 | `markdownLinkRegex` | `\[([^\]]+)\]\(([^)]+)\)` | g | BaseViewRenderer.ts | 313 | `"[Link](url)"` | 匹配Markdown链接 | ❌ |
| 35 | `urlRegex` | `(https?:\/\/[^\s<>"\)]+)` | g | BaseViewRenderer.ts | 314 | `"https://example.com"` | 匹配URL链接 | ❌ |
