...
- 右侧：功能区，内容随视图变化：
  - 日历视图：显示上一期、今天、下一期导航按钮。
  - 任务视图：显示筛选按钮（全部/未完成/已完成）、全局筛选状态文本（Global Filter: ...）、刷新任务按钮。
...
**任务筛选控制（工具栏右侧功能区）**
- 出现在插件主页工具栏的右边区域（功能区），仅在任务视图中显示
- **全部**按钮：显示所有任务
- **未完成**按钮：仅显示未完成的任务
- **已完成**按钮：仅显示已完成的任务
- 当前活跃的筛选按钮高亮显示
- 显示当前全局筛选状态：文案为“Global Filter: ...”
- 提供“刷新任务”按钮以重新加载任务列表

每条任务显示为一行, 包含:复选框,任务描述,优先级图标(显示为Tasks任务格式优先级图标),时间属性,任务位置.
