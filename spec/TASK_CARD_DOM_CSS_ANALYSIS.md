...
1. **Week/Month view缺少 `.pending` 状态规则**
2. **Gantt view完全没有border-left定义**
3. **CSS规则重复定义，造成样式覆盖混乱**
4. **类名设计不统一，难以维护**

推荐采用**方案B（高特指性选择器）**作为短期修复方案，后续版本实施**长期重构方案（统一类名）**以彻底解决架构问题。
