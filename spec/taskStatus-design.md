...
├── views/
│   ├── BaseCalendarRenderer.ts      [修改] 更新复选框渲染
...
**修改 `src/settings.ts`** - 添加 `taskStatuses` 设置项
...
| 文件 | 修改内容 |
|------|----------|
| `src/views/BaseCalendarRenderer.ts` | - 导入 `getStatusColor` 和 `DEFAULT_TASK_STATUSES`<br>- 添加 `getStatusColors()` 方法获取状态颜色<br>- 添加 `applyStatusColors()` 方法应用颜色到元素 |
...
| `src/views/BaseCalendarRenderer.ts` | 添加状态颜色方法 | +20 |
...
### 1. BaseCalendarRenderer.ts - 新增方法
